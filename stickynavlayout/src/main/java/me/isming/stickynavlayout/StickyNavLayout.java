...
public class StickyNavLayout extends LinearLayout {
    ...
    private OnHeaderStatusChange mListener;
    ...
    public void setOnHeadStatusChangeListener(OnHeaderStatusChange listener) {
        mListener = listener;
    }
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (Math.abs(dy) > mTouchSlop) {
                    ...
                    if (mInnerScrollView instanceof ScrollView) {
                        ...
                        if ((!isTopHidden && (dy < 0 || mInnerScrollView.getScrollY() == 0))
                        ...
                        if ((!isTopHidden && (dy < 0 || (c != null && c.getTop() ==0))) || //
                                (c != null //
                                        && c.getTop() == 0//
                                        && isTopHidden && dy > 0)) {
                        ...
                        RecyclerView rv = (RecyclerView) mInnerScrollView;
                        if ((!isTopHidden && (dy < 0 || !android.support.v4.view.ViewCompat.canScrollVertically(rv, -1))) || (!android.support.v4.view.ViewCompat.canScrollVertically(rv, -1) && isTopHidden && dy > 0)) {
                            initVelocityTrackerIfNotExists();
                            mVelocityTracker.addMovement(ev);
                            mLastY = y;
                            return true;
    ...
    private void getCurrentScrollView() {
        if (mBottomView instanceof  ViewPager) {
            ViewPager pager = (ViewPager) mBottomView;
            int currentItem = pager.getCurrentItem();
            mInnerScrollView = (ViewGroup) pager.getChildAt(currentItem);
        } else {
            if (mInnerScrollView == null) {
                mInnerScrollView = (ViewGroup) mBottomView;
            }
        }
    ...
    public void scrollTo(int x, int y) {
        ...
        boolean oldStatus = isTopHidden;
        isTopHidden = getScrollY() == mTopViewHeight;

        if (oldStatus != isTopHidden && mListener != null) {
            mListener.onStatusChange(!isTopHidden);
        }
    ...
    public interface OnHeaderStatusChange {
        void onStatusChange(boolean isShow);
    }
